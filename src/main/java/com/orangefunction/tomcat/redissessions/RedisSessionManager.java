...
public class RedisSessionManager extends ManagerBase implements Lifecycle
{
    ...
    private static final String SESSION_NAMESPACE = "tcsession";
    ...
    public Session createSession(String requestedSessionId)
    {
        ...
        try
        {
            ...
            if(null != requestedSessionId)
            {
                ...
                if(jedis.setnx(generateSessionRedisKey(sessionId).getBytes(), NULL_SESSION) == 0L)
                {
                    sessionId = null;
                }
            }
            else
            {
                ...
                while(jedis.setnx(generateSessionRedisKey(sessionId).getBytes(), NULL_SESSION) == 0L); // 1 = key set; 0 = key already existed
            }
    ...
    public byte[] loadSessionDataFromRedis(String id) throws IOException
    {
        ...
        try
        {
            ...
            jedis = acquireConnection();
            byte[] data = jedis.get(generateSessionRedisKey(id).getBytes());
            error = false;
    ...
    protected boolean saveInternal(Jedis jedis, Session session, boolean forceSave) throws IOException
    {
        ...
        try
        {
            ...
            byte[] binaryId = generateSessionRedisKey(redisSession.getId()).getBytes();
    ...
    private String generateSessionRedisKey(String id)
    {
        return String.format("%s:%s#%s", applicationName, SESSION_NAMESPACE, id);
    }
